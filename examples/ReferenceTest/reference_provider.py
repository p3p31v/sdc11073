...
def get_network_adapter() -> network.NetworkAdapter:
    ...
    if (ip := "127.0.0.1") is not None:  # noqa: SIM112 # if (ip := os.getenv('rec_ip'))
...
def create_reference_provider(
    ...
    dpws_device = dpws_device or ThisDeviceType(friendly_name='TestDevice',
                                                ...
                                                serial_number='12345')
    mdib = ProviderMdib.from_mdib_file(str(mdib_path or pathlib.Path(__file__).parent.joinpath('C:/Users/iccas/Python_Projekte/sdc11073/examples/ReferenceTest/reference_mdib_original.xml')))
